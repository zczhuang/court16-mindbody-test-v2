...
function IntroInner() {
  ...
  const [staffAssistResult, setStaffAssistResult] = useState<{
    firstName: string;
    email: string;
  ...
  async function handleAdultSubmit(request: AdultRequest) {
    ...
    if (data.status === "pending_staff_assist") {
      // Staff-assist offers (e.g. Pickleball BOGO) skip the MindBody cart.
      // Render the "we'll reach out" confirmation inline — no redirect.
      setShowFormModal(false);
      setStaffAssistResult({
        firstName: request.adult.firstName,
        email: request.adult.email,
        offerDisplayName: offer?.displayName ?? "Court 16 session",
  ...
  if (staffAssistResult) {
    return (
      <>
        <Header />
        <StaffAssistConfirmation {...staffAssistResult} />
      </>
    );
  }
